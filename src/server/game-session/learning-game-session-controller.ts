...
import { assertGameCanRender } from "./can-game-render-task";
import { playableNeedsFromPlan } from "./filter-playable-needs";
...
export interface LearningGameSessionDeps {
...
export type RangerTrialSessionDeps = Omit<LearningGameSessionDeps, "definition">;

function publicProgress(record: GameSessionRecord): GamePublicSession {
  return {
...
export class LearningGameSessionController {
  ...
  constructor(private readonly deps: LearningGameSessionDeps) {
    ...
    this.schedulerRandom =
      ...
      ((sessionId) =>
        createSchedulerRandom(this.definition.gameType, sessionId));
    this.taskRandom =
  ...
  async submit(input: {
    ...
    if (record.currentTaskId !== input.taskId) {
      throw new GameSessionError(
        "TASK_NOT_FOUND",
        "Submitted task is not the current game session task",
  ...
  async continue(sessionId: string): Promise<ContinueGameSessionResult> {
    return this.continueWithRetry(sessionId, MAX_SESSION_CONFLICT_RETRIES);
  }

  async resume(sessionId: string): Promise<ResumeGameSessionResult> {
  ...
  private async continueWithRetry(
    sessionId: string,
    retriesLeft: number,
  ): Promise<ContinueGameSessionResult> {
    try {
  ...
  private async resolveContinueFromLatest(
    latest: GameSessionRecord,
  ): Promise<ContinueGameSessionResult | null> {
  ...
  private async resumeCurrentTask(
    record: GameSessionRecord,
  ): Promise<ResumeGameSessionResult> {
    if (!record.currentTaskId) {
