...
import {
  ...
  parseDailyTrainingRecord,
  serializeDailyTrainingState,
  sessionStatusFromPhase,
} from "./daily-training-session-state";
import type {
...
export class SupabaseDailyTrainingSessionStore
  implements DailyTrainingSessionStore
{
  constructor(
    private readonly client: SupabaseClient,
    private readonly options: { expectedUserId?: string } = {},
  ) {}
  ...
  async create(
    ...
    const state = serializeDailyTrainingState(record);
    assertNoAnswerKeyFields(state);
    const { error } = await this.client.from("game_sessions").insert({
      id: record.sessionId,
      user_id: record.userId,
      game_type: DAILY_TRAINING_ORCHESTRATION_TYPE,
  ...
  async save(
    ...
    const { data, error } = await this.client
      ...
      .eq("game_type", DAILY_TRAINING_ORCHESTRATION_TYPE)
  ...
  async get(sessionId: string): Promise<DailyTrainingSessionRecord | null> {
    ...
    return parseDailyTrainingRecord({
      sessionId: row.id,
      userId: row.user_id,
