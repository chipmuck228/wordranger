...
import { V1_PLACEHOLDER_USER_ID, WORD_BUBBLE_GAME_TYPE } from "@/server/auth/v1-user";
...
export function createSupabaseWordBubbleRuntime(input?: {
  ...
  if (!client) {
    throw new GameSessionError(
      ...
      "Word Bubble production runtime requires Supabase configuration",
    );
  }
  ...
  const sessions = new SupabaseGameSessionStore(client, {
    ...
    expectedGameType: WORD_BUBBLE_GAME_TYPE,
  });
