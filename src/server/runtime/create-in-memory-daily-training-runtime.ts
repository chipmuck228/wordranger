...
import { DAILY_TRAINING_TASK_COUNT } from "@/server/auth/v1-user";
...
import { InMemoryDailyTrainingSessionStore } from "@/server/training/in-memory-daily-training-session-store";
import type { DailyTrainingSessionStore } from "@/server/training/daily-training.types";
import type { selectRendererForTask } from "@/server/training/renderer-selector";

export interface CreateInMemoryDailyTrainingRuntimeOptions {
  userId: string;
  now?: () => string;
...
export interface DailyTrainingRuntime {
  vocabulary: VocabularyRepository;
  learning: LearningRepository;
  learningStateQuery: LearningStateQueryRepository;
  tasks: LearningTaskRepository;
  sessions: DailyTrainingSessionStore;
  createController(): DailyTrainingController;
}

export interface InMemoryDailyTrainingRuntime extends DailyTrainingRuntime {
  learning: InMemoryLearningRepository;
...
export function createInMemoryDailyTrainingRuntime(
  options: CreateInMemoryDailyTrainingRuntimeOptions,
): InMemoryDailyTrainingRuntime {
  const vocabulary =
    options.vocabulary ??
  ...
  const sessions = options.sessions ?? new InMemoryDailyTrainingSessionStore();
  ...
  function createController(): DailyTrainingController {
    return new DailyTrainingController({
      ...
      requestedNeedCount: options.requestedNeedCount ?? DAILY_TRAINING_TASK_COUNT,
