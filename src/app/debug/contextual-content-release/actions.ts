...
export async function publishReleaseAction(input: {
  releaseId: string;
  revision: number;
}): Promise<{
  ok: boolean;
  ...
  if (
    ...
    return { ok: false, code: "RELEASE_INVALID", message: "Invalid publish payload." };
  }
  const result = await publishContextualContentRelease({
    releaseId: input.releaseId,
    revision: input.revision,
  });
  const workspace = await projectReleaseWorkspace();
...
export async function rollbackActiveReleaseAction(input: {
  sceneId: string;
  ...
  if (
    ...
    return { ok: false, code: "RELEASE_INVALID", message: "Invalid rollback payload." };
  }
  const result = await rollbackContextualContentActiveRelease({
