...
export function ReleaseWorkspace({
  ...
  async function run(
    ...
    setMessage(result.ok ? "已完成本地验证，未切换 Context Lab。" : result.message ?? "操作失败");
  }
    ...
    <div className="mx-auto flex min-h-full w-full max-w-5xl flex-col gap-6 overflow-x-hidden px-4 py-8">
      ...
      <section className="space-y-3" aria-labelledby="draft-heading">
        ...
        {draft ? (
          <dl className="grid gap-2 text-sm md:grid-cols-2">
            <div>
              <dt className="text-muted-foreground">releaseId</dt>
              <dd data-testid="release-id">{draft.releaseId}</dd>
            </div>
            <div>
              <dt className="text-muted-foreground">status</dt>
              <dd data-testid="release-status">{draft.status}</dd>
            </div>
            <div>
              <dt className="text-muted-foreground">revision</dt>
              <dd data-testid="release-revision">{draft.revision}</dd>
            </div>
            <div>
              <dt className="text-muted-foreground">base release</dt>
              <dd data-testid="release-base">{draft.baseReleaseId ?? "none"}</dd>
            </div>
            <div>
              <dt className="text-muted-foreground">target数量</dt>
              <dd data-testid="release-target-count">{draft.targetEntries.length}</dd>
            </div>
            <div className="md:col-span-2">
              <dt className="text-muted-foreground">target顺序</dt>
              <dd data-testid="release-target-order">
                {draft.targetEntries.map((item) => item.displayLabel).join(" → ")}
              </dd>
            </div>
            <div className="md:col-span-2">
              <dt className="text-muted-foreground">pack fingerprint</dt>
              <dd data-testid="release-pack-fingerprint">
                <Fingerprint value={workspace.fingerprints.packFingerprint} />
              </dd>
            </div>
            <div className="md:col-span-2">
              <dt className="text-muted-foreground">context fingerprint</dt>
              <dd data-testid="release-context-fingerprint">
            ...
            <div className="md:col-span-2">
              ...
              <dd data-testid="release-fingerprint">
                <Fingerprint value={workspace.fingerprints.releaseFingerprint} />
              </dd>
            </div>
          </dl>
        ) : (
      ...
      <section className="space-y-3" aria-labelledby="actions-heading">
        ...
        <div className="flex min-w-0 flex-wrap gap-2">
          ...
          <Button
            type="button"
            variant="secondary"
            disabled={!workspace.writeEnabled || busy || !draft}
            onClick={() =>
              run(() =>
                preflightReleaseAction({
        ...
        <p data-testid="release-no-publish" className="text-muted-foreground text-sm">
          本阶段没有发布、激活、回滚或写入 /train。
        </p>
      </section>
    </div>
  );
