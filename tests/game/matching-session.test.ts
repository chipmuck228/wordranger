...
import { canMatchingRenderTask } from "@/server/game-session/matching-capability";
...
describe("Matching session", () => {
  it("M14: start Matching returns a playable CHOICE task", async () => {
    ...
    expect(canMatchingRenderTask(started.task)).toBe(true);
  ...
  it("M15: submit Matching action writes Evidence via submitTaskAction", async () => {
    const world = createMatchingWorld();
    const started = await world.controller.start();
    ...
    expect(evidence[0].gameId).toBe(MATCHING_GAME_ID);
  });

  it("M16: continue returns the next Matching-compatible task", async () => {
    const world = createMatchingWorld();
    const started = await world.controller.start();
    ...
    expect(canMatchingRenderTask(continued.task!)).toBe(true);
    expect(continued.task?.id).not.toBe(started.task.id);
  });

  it("M17: resume after refresh restores the same current task", async () => {
    const world = createMatchingWorld();
    const started = await world.controller.start();
  ...
  it("M18: game_type MATCHING is persisted", async () => {
    const shared = new Map();
    const sessions = new InMemoryGameSessionStore(MATCHING_GAME_TYPE, shared);
    const world = createMatchingWorld("matching-type-user", { sessions });
    const started = await world.controller.start();
    expect(shared.get(started.session.sessionId)?.gameType).toBe(
      MATCHING_GAME_TYPE,
    );
    const record = await world.sessions.get(started.session.sessionId);
    expect(record?.sessionId).toBe(started.session.sessionId);
  });

  it("M19: distinct MATCHING gameId appears in Evidence", async () => {
    const world = createMatchingWorld();
    const started = await world.controller.start();
    await world.controller.submit({
      sessionId: started.session.sessionId,
  ...
  it("M20: double continue produces one next task", async () => {
    const world = createMatchingWorld();
    const started = await world.controller.start();
    await world.controller.submit({
      sessionId: started.session.sessionId,
  ...
  it("M21: concurrent submit produces one Evidence", async () => {
    const world = createMatchingWorld();
    const started = await world.controller.start();
    const payload = {
      sessionId: started.session.sessionId,
  ...
  it("M22: resume generation race produces one current task", async () => {
    const world = createMatchingWorld();
    const started = await world.controller.start();
  ...
  it("rejects TEXT_INPUT even after need filtering", async () => {
    ...
    expect(canMatchingRenderTask(unrenderable)).toBe(false);
    expect(canMatchingRenderTask(meaningChoiceTask())).toBe(true);
    expect(
      canMatchingRenderTask(
        makePublicTask({
