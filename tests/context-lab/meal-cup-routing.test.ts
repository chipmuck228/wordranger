...
describe("Meal Context Lab cup routing after experiment promotion", () => {
  it("exposes exactly five stable Probe targets ending in cup", () => {
    ...
    expect(targets.some((item) => item.entityId.includes("plate"))).toBe(false);
    expect(targets.some((item) => item.target.senseId === "drink#consume-liquid")).toBe(false);
    expect(targets.at(-1)?.target).toEqual(MEAL_SCENE_EXPANSION_BATCH_01_CUP_TARGET);
  });
