...
| `provisional-word-placement.json` | One INFERRED assignment per canonical lexeme, generated by `npm run generate:provisional-placement`. |
...
Provenance is `INFERRED` with `wordranger-provisional-band-generator/v1`. Records are **not** written onto production `curriculumBand` / `gradeBand` / `difficultyBand` / `frequencyBand`, so they cannot satisfy `assessAdaptivePlacementReadiness()`. Future human review (Step C) can create `CURATED` overrides without changing canonical `lexemeId` or learner history.

Regenerate with unchanged canonical input for byte-equivalent output. Production Adaptive Placement remains `PLACEMENT_DATA_BLOCKER` until a reviewed overlay exists.
...
Provisional review buckets are a separate `VocabularyPlacementProvider` (`listProvisionalPlacements()`, `getBandDefinition()`). Daily Training and Scheduler must not call it. Future curated data can replace the file-backed provider without changing Evidence, `StudentLexemeModel`, TaskEvaluator, or renderers.
