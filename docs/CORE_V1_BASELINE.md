...
Phase 05 (Ranger Trial) found no Core integration blocker. Phase 06 (Word Bubble), Phase 07 (Matching), Phase 08 (Snake), and Phase 09 (Daily Training) consume the same frozen baseline. Daily Training is orchestration and UX only: it does not change mastery, retention, weakness, evidence, need, scheduler, evaluator, or vocabulary-policy behavior. Snake `responseTimeMs` includes navigation overhead; Core still consumes that field for `SLOW_RESPONSE` (see ADR-063 / `CORE_MEASUREMENT_CONCERN`). Daily Training must not compare raw response time across renderer types. Do not change Core thresholds in this phase.
