...
- Placement metadata lives in Vocabulary Domain (`listPlacementMetadata()`), not on `StudentLexemeModel` / Evidence / `game_sessions` / `learning_tasks`. Every placement field needs provenance (`SOURCE` / `CURATED` / `EXTERNAL_REFERENCE` / `INFERRED`). `INFERRED` is not production placement authority. Adaptive placement stays `PLACEMENT_DATA_BLOCKER` until a `CURATED` or `EXTERNAL_REFERENCE` band overlay passes `assessAdaptivePlacementReadiness()`. Do not rank, skip, or jump Scheduler admission using `sourceIndex`, `alphabeticalSection`, `starred`, or `functionWord`. Neutral provisional review buckets (`BAND_1`…`BAND_6`) are a separate INFERRED scaffolding layer; they are not curriculum/grade/difficulty truth and must not unlock that gate. Placement records are keyed by real `Lexeme.id`. `canonicalKey` is only a stable vocabulary identifier for normalization/debugging, not the placement foreign key. Human-reviewed overrides are stored separately as CURATED records; effective placement is curated-if-present else provisional. Scheduler and Daily Training must not consume that review layer until a later contract-changing step. Curated metadata never rewrites learner Evidence or `StudentLexemeModel`. The `/debug/vocabulary-placement` review UI is internal/local tooling and must not appear in student navigation. Hiding that route is **not** authorization. Until admin auth exists, review writes require an explicit `PLACEMENT_REVIEW_WRITE_ENABLED=1` server gate. `CuratedPlacementStore` is the persistence boundary: `file` is local/internal only; deployed persistence is the Supabase adapter and must use the service role (never the anon key). Scheduler and Daily Training must not query `vocabulary_placement_reviews`.
