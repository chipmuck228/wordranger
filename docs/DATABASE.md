...
- `answer_key` — **server-only**. Never `select("*")` this table from a browser client. Evaluation must run on the server via `submitTaskAction` (Debug Lab may display the key). Dedicated V0 inlines the same six-table RLS ENABLE (not FORCE), client revoke, and `service_role` DML grants in `supabase/migrations/202609260001_dedicated_wordranger_baseline_v0.sql`. The historical hardening file is archived at `supabase/migrations_archive/pre_dedicated_baseline/202609250001_learner_table_server_only_access.sql`. No client policies. `getTaskForEvaluation` must bind `taskId + userId + sessionId` before selecting `answer_key`. The historical file was applied through the authorized Dashboard SQL Editor; apply catalog and runtime verification are in `docs/FREE_PRACTICE_SECURITY_MIGRATION_APPLY_EVIDENCE.md`. Dedicated remote apply of the V0 baseline has **not** happened. Migration history remains absent. `db push` remains forbidden. `/practice` remains disabled. Candidate / not a Standard. Deployment status is documented here, not in database object comments.
