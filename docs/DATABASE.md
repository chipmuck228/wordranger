...
- `answer_key` — **server-only**. Never `select("*")` this table from a browser client. Evaluation must run on the server via `submitTaskAction` (Debug Lab may display the key). `202609250001_learner_table_server_only_access.sql` enables RLS on the six `public` learner tables (`public.learning_tasks` and the other five) and revokes `anon` / `authenticated`. No client policies. `getTaskForEvaluation` must bind `taskId + userId + sessionId` before selecting `answer_key`. That migration was applied through the authorized Dashboard SQL Editor; apply catalog and runtime verification are in `docs/FREE_PRACTICE_SECURITY_MIGRATION_APPLY_EVIDENCE.md`. Migration history remains absent. `db push` remains forbidden. `/practice` remains disabled. Candidate / not a Standard. Deployment status is documented here, not in database object comments.
